...
const Notis: React.FC = () => {
  ...
  useEffect(() => {
    const fetchNotificaciones = async () => {
      ...
      try {
        const url = `https://apiss-81oo.onrender.com/api/notificaciones/${departamento}`;
  ...
  const handleMarkAsRead = async (id: string) => {
    try {
      const response = await axios.patch(`https://apiss-81oo.onrender.com/api/notificaciones/${id}`, { leido: true });
